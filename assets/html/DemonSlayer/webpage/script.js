...
["#graph"].forEach((id) => {
  d3.select(id).attr("width", svgWidth).attr("height", svgHeight).style("border", "1px solid black");
});
...
d3.select("#highlightBtn").on("click", () => {
  ...
  graphSvg.selectAll("circle.node").classed("selected", (d) => topNodeIds.has(d.id));
});
...
function updatePercolation(timestep) {
  ...
  graphSvg.selectAll("circle.node").each(function (d) {
...
function updateVoterankStep(step) {
  ...
  graphSvg.selectAll("circle.node").each(function (d) {
    ...
    d3.select(this).classed("people", false).classed("influencer", false).classed("crowd", false);
