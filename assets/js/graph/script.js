...
const adjMatrix = Array.from({ length: nodeIds.length }, () => Array(nodeIds.length).fill(0));
// Populate the matrix
links.forEach((link) => {
...
gridGroups.append("rect").attr("width", gridCellSize).attr("height", gridCellSize).attr("class", "cell");
...
// Draw links
const link = graphSvg.append("g").selectAll("line").data(links).enter().append("line").attr("class", "link");

// Group node circle + label together
const nodeGroup = graphSvg
...
nodeGroup.append("circle").attr("class", "node").attr("r", nodeRadius);
...
function highlightNode(nodeId) {
  gridSvg.selectAll("rect.cell").classed("highlight", (d) => d.id === nodeId);

  // Highlight edges in the grid
  gridSvg.selectAll("line.grid-link").classed("highlight", (l) => l.source === nodeId || l.target === nodeId);

  matrixSvg.selectAll("rect.matrix-cell").classed("highlight", (d) => {
    const rowIdx = nodeIds.indexOf(d.row.id);
    const colIdx = nodeIds.indexOf(d.col.id);
  ...
  matrixSvg.selectAll("text.row-label").classed("highlight", (d) => d === nodeId);
  matrixSvg.selectAll("text.col-label").classed("highlight", (d) => d === nodeId);
  ...
  graphSvg.selectAll("line.link").classed("highlight", (l) => l.source.id === nodeId || l.target.id === nodeId);

  // Graph: highlight node label
  graphSvg.selectAll("text.node-label").classed("highlight", (d) => d.id === nodeId);

  // Highlight grid links connected to this node
  gridSvg.selectAll(".grid-link").classed("highlight", (l) => l.source.id === nodeId || l.target.id === nodeId);
}

function highlightEdge(sourceId, targetId) {
  // Graph: highlight node label
  graphSvg.selectAll("text.node-label").classed("highlight", (d) => d.id === sourceId || d.id === targetId);

  graphSvg
    .selectAll("line.link")
    .classed("highlight", (l) => (l.source.id === sourceId && l.target.id === targetId) || (l.source.id === targetId && l.target.id === sourceId));

  // 🌟 Grid: highlight the connecting edge
  gridSvg
    .selectAll("line.grid-link")
    .classed("highlight", (l) => (l.source.id === sourceId && l.target.id === targetId) || (l.source.id === targetId && l.target.id === sourceId));

  // 🌟 Optionally highlight the two grid nodes as well
  gridSvg.selectAll("rect.cell").classed("highlight", (d) => d.id === sourceId || d.id === targetId);

  // Matrix: highlight the **exact matrix cell**
  matrixSvg.selectAll("rect.matrix-cell").classed("highlight", (d) => {
    return (d.row.id === sourceId && d.col.id === targetId) || (d.row.id === targetId && d.col.id === sourceId);
  });
}
